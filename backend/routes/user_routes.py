...
async def create_user(user: UserCreate, admin: dict = Depends(role_required("Admin")), db: Session = Depends(get_db)):
    return user_controller.create_user(db, user)

@router.get("", response_model=List[UserOut])
async def list_users(role: Optional[str] = None, q: Optional[str] = None, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_user(user_id: int, user_update: UserUpdate, admin: dict = Depends(role_required("Admin")), db: Session = Depends(get_db)):
    return user_controller.update_user(db, user_id, user_update)

@router.patch("/{user_id}/status", response_model=UserOut)
async def deactivate_user(user_id: int, status_update: UserStatusUpdate, admin: dict = Depends(role_required("Admin")), db: Session = Depends(get_db)):
