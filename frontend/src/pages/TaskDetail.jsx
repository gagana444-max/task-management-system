...
export default function TaskDetail() {
  ...
  const fetchComments = async () => {
    try {
      const res = await api.get(`/comments/tasks/${id}`, { headers })
  ...
  const fetchAttachments = async () => {
    try {
      const res = await api.get(`/comments/tasks/${id}/attachments`, { headers })
  ...
  const postComment = async (e) => {
    ...
    try {
      setPosting(true)
      const res = await api.post(`/comments/tasks/${id}`, { content: newComment.trim() }, { headers })
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#f5f4f0' }}>
      <div style={{ width: 32, height: 32, border: '4px solid #818cf8', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    </div>
  )
  ...
  return (
    <div style={{ fontFamily: "'Instrument Sans', sans-serif", padding: 22, minHeight: '100vh', background: '#f5f4f0' }}>
      ...
      {/* Back */}
      <button onClick={() => navigate(-1)} style={{ fontSize: 12, color: '#818cf8', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 4 }}>
        ← Back to Task Board
      </button>
      ...
      {error && (
        <div style={{ marginBottom: 14, padding: '10px 14px', background: '#fff5f5', border: '1.5px solid #fecaca', borderRadius: 10, fontSize: 12, color: '#dc2626', display: 'flex', justifyContent: 'space-between' }}>
          {error} <button onClick={() => setError('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fca5a5' }}>✕</button>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #e8e8f0', overflow: 'hidden' }}>
            <div style={{ height: 4, background: strip(task.status) }} />
            <div style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 9, padding: '3px 9px', borderRadius: 7, fontWeight: 700, background: spill(task.status).bg, color: spill(task.status).color, marginLeft: 10, flexShrink: 0 }}>{slabel(task.status)}</span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ...
                  { label: 'Due Date', val: <span style={{ fontSize: 12, color: isLate(task.due_date, task.status) ? '#dc2626' : '#1a1a2e', fontWeight: 500 }}>{fmt(task.due_date)}</span> },
                  { label: 'Assigned To', val: task.assigned_user ? <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 20, height: 20, borderRadius: '50%', background: avc(task.assigned_user.name), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 700, color: '#fff' }}>{ini(task.assigned_user.name)}</span><span style={{ fontSize: 12, color: '#1a1a2e', fontWeight: 500 }}>{task.assigned_user.name}</span></span> : <span style={{ fontSize: 12, color: '#9090a0' }}>Unassigned</span> },
          ...
          {canEdit && (
            <div style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #e8e8f0', padding: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {task.status !== 'todo' && <button onClick={() => moveTask('todo')} style={{ flex: 1, padding: '8px', borderRadius: 7, fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: "'Instrument Sans', sans-serif", border: '1.5px solid #fde68a', background: '#fef9e7', color: '#d97706' }}>To Do</button>}
                {task.status !== 'in_progress' && <button onClick={() => moveTask('in_progress')} style={{ flex: 1, padding: '8px', borderRadius: 7, fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: "'Instrument Sans', sans-serif", border: '1.5px solid #bfdbfe', background: '#eff6ff', color: '#2563eb' }}>In Progress</button>}
                {task.status !== 'completed' && <button onClick={() => moveTask('completed')} style={{ flex: 1, padding: '8px', borderRadius: 7, fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: "'Instrument Sans', sans-serif", border: '1.5px solid #a7f3d0', background: '#ecfdf5', color: '#059669' }}>Completed</button>}
              </div>
            </div>
          )}
        </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ padding: 14, borderBottom: '1px solid #f0f0f8', flexShrink: 0 }}>
                <form onSubmit={postComment}>
                  <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Write a comment..." rows={2}
                    style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1.5px solid #e8e8f0', fontSize: 12, fontFamily: "'Instrument Sans', sans-serif", outline: 'none', resize: 'none', marginBottom: 8 }} />
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {comments.length === 0 ? (
                  ...
                  <div key={c.id} style={{ background: '#f8f8fc', borderRadius: 10, padding: '10px 12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <div style={{ width: 22, height: 22, borderRadius: '50%', background: avc(c.user?.name || ''), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 700, color: '#fff' }}>{ini(c.user?.name || '')}</div>
                      <span style={{ fontSize: 11, fontWeight: 600, color: '#1a1a2e' }}>{c.user?.name || 'Unknown'}</span>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {attachments.length === 0 ? (
                  ...
                  <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f8f8fc', borderRadius: 9, padding: '10px 12px', border: '1px solid #f0f0f8' }}>
                    <span style={{ fontSize: 16 }}>📎</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 500, color: '#1a1a2e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.filename}</div>
                      {a.size && <div style={{ fontSize: 10, color: '#b0b0c0' }}>{formatSize(a.size)}</div>}
                    </div>
                    <a href={`http://localhost:8000/api/comments/tasks/${id}/attachments/${a.id}/download`} target="_blank" rel="noreferrer"
